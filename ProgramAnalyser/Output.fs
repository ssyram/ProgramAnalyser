...
let printJoinedDestEdge (guard, lst) =
    let guard = conjCmpsToGeConj LossConfirm guard in
    let printVarRange (var, lower, upper) =
        $"{var}:[{lower}, {upper}]"
    in
    let printLoc (loc, varRanges) =
        [
            $"dest:{loc}"
            "var-ranges:"
            String.concat "\n" $ List.map printVarRange varRanges
            "end-var-ranges"
        ]
        |> fromListGenOutput
    in
...
type CfgItem =
    { updates : Map<Variable, ArithExpr>
      ...
      /// the edges that have a single destination location
      /// `[(location, guard)]`
      /// where the `guard` is the condition of going to this location via this edge
      singleDestEdges : (Location * ConjCmps) list
      /// The hyper-edges that have multiple destination locations.
      /// depending on the range of the random variables:
      /// `[(guard, [(location, [(variable, lower, upper)])])]`
      /// where the `guard` is the condition taking this hyper-edge,
...
type Output =
    { // The metadata to declare
      ...
      // the CFG information
      cfgItems : CfgItem list
    }

type private Analyser(name : string, program : Program) = class
